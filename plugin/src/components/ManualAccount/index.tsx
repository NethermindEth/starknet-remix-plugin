...
const ManualAccount: React.FC = () => {
	...
	useEffect(() => {
		...
		if (manualAccounts != null && accounts.length === 0 && selectedAccount == null) {
	...
	useEffect(() => {
		if (selectedAccount != null && account != null) {
			...
			const accountExist = accounts.find((acc) => acc.address === selectedAccount.address);
		...
		if (accounts.length > 0) {
			...
			if (provider != null) {
				setAccount(new Account(provider, accounts[0].address, accounts[0].private_key));
				setDeclTxHash("");
				setDeployTxHash("");
				setInvokeTxHash("");
			}
	...
	function handleAccountChange(accountIndex: number): void {
		...
		if (provider != null) {
			setAccount(new Account(provider, selectedAccount.address, selectedAccount.private_key));
			setDeclTxHash("");
			setDeployTxHash("");
			setInvokeTxHash("");
		}
		...
		<div className="manual-root-wrapper">
			<div className="network-selection-wrapper">
				<div className={"selector-wrapper"}>
					<Select.Root
						value={selectedAccount != null ? selectedAccount.address : ""}
								...
								<div className="flex flex-row justify-content-space-between align-items-center p-0">
									<Select.Value placeholder="Select Account">
										{selectedAccount != null
											? trimStr(selectedAccount.address, 6)
											: "Select Account"}
									</Select.Value>
									<Select.Icon>
										<div>
			...
			{selectedAccount != null && account != null && provider != null && (
				<div className="info-box-manual-account">
					...
					<span className={"info-box-value"}>
						{parseFloat(ethers.utils.formatEther(selectedAccount.balance))?.toFixed(8)}{" "}
						ETH
					</span>
					<button
						className="btn info-box-copy-btn"
			...
			{selectedAccount != null && (networkName === "goerli" || networkName === "sepolia") && (
				<button
					...
					onClick={() => {
						copy(selectedAccount?.address ?? "");
						remixClient
							.call("notification" as any, "toast", "ℹ️ Address copied to Clipboard")
							.catch((err) => {
								console.log(err);
							});
						setTimeout(() => {
					...
					<Select.Content>
						<Select.Viewport>
							{networkConstants.map((network) => (
								...
								>
									<Select.ItemText>{network.value}</Select.ItemText>
								</Select.Item>
							))}
						</Select.Viewport>
					</Select.Content>
