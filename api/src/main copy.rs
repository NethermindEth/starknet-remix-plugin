...
#[derive(Responder)]
#[response(content_type = "application/x-compilation-sierra")]
struct SierraCompilationResponse {
    file_name: Option<NamedFile>, 
    compilation_message: Header<'static>,
    compilation_status: Header<'static>,
}



#[post("/compile-to-sierra", data = "<file>")]
async fn compile_to_sierra(file: Data<'_>) -> SierraCompilationResponse {
    ...
    println!("LOG: Compilation output: {:?}", output);

    SierraCompilationResponse {
        file_name : NamedFile::open(file_hash.sierra_path()).await.ok(), 
        compilation_message : Header::new("X-CompilationMessage", String::from_utf8(output.stderr).unwrap()),
        compilation_status: match output.status.code() {
            Some(0) => Header::new("X-CompilationStatus", "Success".to_string()),
            Some(_) => Header::new("X-CompilationStatus", "Failure".to_string()),
            None => Header::new("X-CompilationStatus", "Failure".to_string()),
        }
    }
